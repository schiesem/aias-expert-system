...
output_log = []  # Hier speichern wir die Ausgaben, die normalerweise ins Terminal gehen würden
...
# Die Reasoner-Archive (HermiT, Pellet) werden mit dem installierten
# owlready2-Paket ausgeliefert. Der Klassenpfad wird daher aus dessen
# Verzeichnis abgeleitet und nicht aus dem dieser Datei.
_HERE = os.path.dirname(owlready2.__file__)
...
def sync_reasoner_pellet(x = None, infer_property_values = False, infer_data_property_values = False, debug = 1, keep_tmp_file = False, report=1):  
  if   isinstance(x, World):    world = x
  elif isinstance(x, Ontology): world = x.world
  elif isinstance(x, list):     world = x[0].world
  else:                         world = owlready2.default_world
  ...
  try:
    ...
    if debug or report:
        ...
        if debug:
            ...
            print("    %s" % " ".join(command), file=sys.stderr)
        if report:
            output_log.append("* Owlready2 * Running Pellet...")  # HINZUGEFÜGT
    ...
    try:
      ...
      if (e.returncode == 1) and (b"ERROR: Ontology is inconsistent" in (e.stderr or b"")): # XXX
        ...
        # Run pellet explain to get details about inconsistency
        # Try different approaches to get explanation
        explain_output = ""

        # Approach 1: pellet explain --inconsistent (Pellet 2.x syntax)
        command_explain = [owlready2.JAVA_EXE, "-Xmx%sM" % JAVA_MEMORY, "-cp", _PELLET_CLASSPATH, "pellet.Pellet", "explain", "--inconsistent", tmp.name]
        process = subprocess.run(command_explain, stdout = subprocess.PIPE, stderr = subprocess.PIPE, check = False, **_subprocess_kargs)
        explain_output = _decode(process.stdout) + _decode(process.stderr)

        # Approach 2: If that fails, try pellet consistency (shows why inconsistent)
        if not explain_output.strip() or "Usage:" in explain_output or "Error" in explain_output:
            command_consistency = [owlready2.JAVA_EXE, "-Xmx%sM" % JAVA_MEMORY, "-cp", _PELLET_CLASSPATH, "pellet.Pellet", "consistency", "-v", tmp.name]
            process2 = subprocess.run(command_consistency, stdout = subprocess.PIPE, stderr = subprocess.PIPE, check = False, **_subprocess_kargs)
            explain_output = _decode(process2.stdout) + _decode(process2.stderr)

        msg += "\n\n=== PELLET EXPLAIN OUTPUT ===\n{}\n".format(explain_output if explain_output.strip() else "(No detailed explanation available)")
        if debug:
          print("* Owlready2 * Pellet explain output:", file=sys.stderr)
          print(explain_output, file=sys.stderr)
    ...
    if report:
      output_log.append("Pellet Reasonner took %s seconds" % (time.time() - t0)) # HINZUGEFÜGT
  ...
  _apply_reasoning_results(world, ontology, debug, new_parents, new_equivs, entity_2_type, report)
  if infer_property_values:      _apply_inferred_obj_relations (world, ontology, debug, inferred_obj_relations, report)
  if infer_data_property_values: _apply_inferred_data_relations(world, ontology, debug, inferred_data_relations, report)
  
  if debug: 
    print("* Owlready * (NB: only changes on entities loaded in Python are shown, other changes are done but not listed)", file = sys.stderr)
  
  output_result = list(output_log) # <- Ergebnis kopieren
  output_log.clear() # <- Liste leeren
  
  if report:
    return (output_result) # <- Kopie zurückgeben
  else:
    return (None)

  
def _apply_reasoning_results(world, ontology, debug, new_parents, new_equivs, entity_2_type, report):
  ...
  with LOADING: # Because triples were asserted above => only modify Python objects WITHOUT creating new triples!
    for concept1, concepts2 in new_equivs_loaded.items():
      for concept2 in concepts2:
        if debug: 
          print("* Owlready * Equivalenting:", concept1, concept2, file = sys.stderr)
        if report:
          output_log.append(f"Equivalenting: {concept1} == {concept2}")
    ...
    for child, parents in new_parents_loaded.items():
      ...
      if debug: 
        print("* Owlready * Reparenting %s:" % child, old, "=>", new, file = sys.stderr)
        output_log.append(f"Reparenting: {child}, {old} => {new}") # HINZUGEFÜGT
      ...
      for child_eq in child.equivalent_to.indirect():
        if isinstance(child_eq, ThingClass):
          if debug: 
            print("* Owlready * Reparenting %s (since equivalent):" % child_eq, old, "=>", new, file = sys.stderr)
          if report:
            output_log.append(f"Reparenting: {child_eq}, {old} => {new}") #HINZUGEFÜGT
...
def _apply_inferred_obj_relations(world, ontology, debug, relations, report):
  for a_storid, prop, b_storid in relations:
    ...
    if not a is None:
      if debug:
        ...
        if not b is None: 
          print("* Owlready * Adding relation %s %s %s" % (a, prop.name, b))
      if prop._python_name in a.__dict__:
        delattr(a, prop._python_name)
      if report:
        output_log.append(f"AddRelation: {a}, {prop.name}, {b}")
...
def _apply_inferred_data_relations(world, ontology, debug, relations, report):
  for a_storid, prop, value, datatype in relations:
    ...
    if not a is None:
      ...
      if report:
        output_log.append(f"AddRelation: {a}, {prop.name}, {value}")
      if prop._python_name in a.__dict__:
        delattr(a, prop._python_name)
